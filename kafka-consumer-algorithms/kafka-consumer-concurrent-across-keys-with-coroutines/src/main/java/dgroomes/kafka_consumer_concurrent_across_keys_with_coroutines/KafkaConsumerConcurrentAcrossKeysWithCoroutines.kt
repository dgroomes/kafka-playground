...
class KafkaConsumerConcurrentAcrossKeysWithCoroutines(
    ...
    private val tailOffsetJobByPartition = mutableMapOf<TopicPartition, Job>()
    private val nextOffsets = mutableMapOf<TopicPartition, Long>()
    ...
    private fun poll() {
        ...
        while (true) {
            ...
            for (partition in records.partitions()) {
                val recordsForPartition = records.records(partition)
                for (record in recordsForPartition) {
    ...
    private fun commit() {
        ...
        val toCommit = nextOffsets.map { (partition, offset) -> partition to OffsetAndMetadata(offset) }.toMap()
        nextOffsets.clear()
